...
topic_model <- stm(spars_tidy_dfl, K = 3)
...
score_words <- labelTopics(topic_model, n=20, topics = 1:3)
...
doc_topic_t2 <- doc_topic_t %>%
  rowwise() %>%
  mutate(topic_number = which.max(c_across(2:4))) #Alterar de acordo com k
doc_topic_t2

#Juntando com a base de sentimentos
